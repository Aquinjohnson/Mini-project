import streamlit as st
import numpy as np
import pandas as pd
import os
from sklearn.feature_extraction.text import TfidfVectorizer
from sklearn.metrics.pairwise import cosine_similarity
from func import *
from string import punctuation
from nltk.corpus import stopwords

#####
df = pd.read_csv('Processed_data.csv' ,engine='python',encoding='cp1252')
import nltk
nltk.download('stopwords')
stop = stopwords.words('english')
stop = set(stop)

df['clean_title']=df['title'].apply(clean_text)

vectorizer = TfidfVectorizer(analyzer='word', lowercase=False)
X = vectorizer.fit_transform(df['clean_title'])
title_vectors = X.toarray()

tk = 0
st.title('Book Recommendation System')

col1, col2 = st.columns(2)
#taking book name as input
with col1:
    book = st.text_input('Enter book name that you liked : ')
    
#taking multiple fiels to get similarity
with col2:
    feat = st.selectbox("Select Mode : ",['title', 'average_rating', 'authors'])
    if st.button('Search'):
        tk = 1

#st.dataframe(df.head(10))
if tk == 1:
    st.success('Recommending books similar to '+book)
    rec = st.empty()
    rec = st.dataframe(get_recommendations(book, 'title', df, title_vectors, feat), width=700, height=76)

#print(get_recommendations())